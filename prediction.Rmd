...
cartFit <- train(complete ~ ., #Define which variable to predict 
                ...
                method = "rpart", #Define the model type/Rpart uses CART
...
A:ROC was used to select the optimal model using the largest value.
...
A: With the Complexity Parameter going up, the ROC is going down simultaneously. So we need to prune the tree when CP is relatively small and ROC relatively large for the moedel.
...
#it's a pretty good prediciton for the accuracy being as large as 0.905
```
...
A: 1) instead of gini index, C4.5 adopts information gain ratio to classify; 2) also with CART we'll have a binary tree and with C4.5 we can have a split with multiple branches 3) for CART, we use cost-complexity pruning, whereas for C4.5 we use error-based pruning
...
A:ROC was used to select the optimal model using the largest value.
The final values used for the model were C = 0.5 and M = 3. It is quite successful model for the ROC is quite close to 1.0

What does the plot represent? What information does this plot tell us?
A: the confidence threshold and the ROC area are positively correlated and the case where the minimun intances per leaf is 3 obtains the most optimistic predictive power.
...
A: models can be trained at a faster speed; and we can do the pre-selection of helpful attributes before classification(winnowing process)
...
A: Judged from ROC, the third model(C5.0) has the best performance. 
...
```{r}
varImp(c50Fit)
# from it we know that the most important variables are courses_taken, years they've been in their program, and entrance_test_score. So to avoid students droppinp out of courses, we can provide extra help for students who just enter the program and the ones who select just a few courses for one semester or have low score of entrance test.
```
